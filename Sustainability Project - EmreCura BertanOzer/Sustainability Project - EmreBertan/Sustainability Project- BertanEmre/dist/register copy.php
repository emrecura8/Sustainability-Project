...
if ( $_SERVER["REQUEST_METHOD"] === "POST") 
    {
        ...
        if(isset($_POST["submit"]))
        {
            ...
            if(empty($error) && isset($_POST)){
                header("Location: reged copy.php");
            }
        ...
        else if(isset($_POST["submit2"]))
        {
            ...
            {
                $error["city"]="City is empty";
           
            }
       
            if(strlen(trim($_POST["dist"]))===0)
            {
                $error["dist"]="Dist is empty";
           
            }
            if(strlen(trim($_POST["add"]))===0)
            {
                $error["add"]="Address is empty";
                ...
                <div class="cont_info_log_sign_up">
                    <div class="col_md_login">
                        <div class="cont_ba_opcitiy">
                            ...
                            <h2>Firm User</h2>
                            <p>Create your Firm User account.</p>
                            <button class="btn_login" onclick="cambiar_login()">Register</button>
                        </div>
                    </div>
                    <div class="col_md_sign_up">
                        <div class="cont_ba_opcitiy">
                            ...
                            <p>Create your Instructor User or Student or Admin account.</p>
                ...
                <div class="cont_forms">
                    ...
                    <form action="" method="post">
                        <div class="cont_form_login">
                            ...
                            <input type="text" placeholder="City" name="city"
                                value="<?=isset($city) ? filter_var($city, FILTER_SANITIZE_FULL_SPECIAL_CHARS) : "" ?>" />
                            <?php
                                if(isset($error["city"]))
                                {
                                    echo "<p class='error_msg '>{$error["city"]}</p>";
                                }
                                else
                                {
                                    echo "";
                            ...
                            <input type="text" placeholder="District" name="dist"
                                value="<?=isset($dist) ? filter_var($dist, FILTER_SANITIZE_FULL_SPECIAL_CHARS) : "" ?>" />
                            <?php
                                if(isset($error["dist"]))
                                {
                                    echo "<p class='error_msg '>{$error["dist"]}</p>";
                                }
                                else
                                {
                                    echo "";
                            ...
                            <input type="text" placeholder="Address" name="add"
                                value="<?=isset($add) ? filter_var($add, FILTER_SANITIZE_FULL_SPECIAL_CHARS) : "" ?>" />
                            <?php
                                if(isset($error["add"]))
                                {
                                    echo "<p class='error_msg '>{$error["add"]}</p>";
                                }
                                else
                                {
                                    echo "";
                            ...
                            ?>
                            <button class="btn_login" type="submit" name="submit2">Register</button>
                    ...
                    <form action="" method="post">
                        <div class="cont_form_sign_up">
                            ...
                            <br>
                            <div class="label">
                                <label for="test">Instructor</label>
                                <input type="radio" value="Instructor" name="radio"
                                    <?=isset($radio) && $radio=="Instructor" ? "checked" :"" ?>>
                                <label for="test">Project Advisor </label>
                                <input type="radio" value="Project Advisor" name="radio"
                                    <?=isset($radio) && $radio=="Project Advisor" ? "checked" :"" ?>>
                                <label for="test">Student</label>
                                <input type="radio" value="Student" name="radio"
                                    <?=isset($radio) && $radio=="Student" ? "checked" :"" ?>>
                                <label for="test">Admin</label>
                                <input type="radio" value="Admin" name="radio"
                                    <?=isset($radio) && $radio=="Admin" ? "checked" :"" ?>>

                            </div>
                            <br>
                            <button class="btn_sign_up" type="submit" name="submit">Register</button>
